...
def exec_oldconfig(kernel_root, kernel_arch, file_name, kernel_defconfig, action_name):
	...
	if kernel_arch == 'arm64':
		build_config = os.path.join(kernel_root, 'build.config.mtk.aarch64')
	if kernel_arch == 'arm':
		build_config = os.path.join(kernel_root, 'build.config.mtk.arm')
	if '_kasan_' in kernel_defconfig:
		build_config = os.path.join(kernel_root, 'build.config.mtk.aarch64.kasan')
	if '_ubsan_' in kernel_defconfig:
		build_config = os.path.join(kernel_root, 'build.config.mtk.aarch64.ubsan')
	kernel_cmd = ''
	if os.path.exists(build_config):
		kernel_cmd = './device/mediatek/build/build/tools/build_defconfig.sh %s %s %s %s %s' % (kernel_root, kernel_arch, build_config, kernel_out, action_name)
	else:
...
def modify_exec_oldconfig(kernel_root, kernel_arch, file_name, option_list, action_name):
	...
	res_cmd = exec_oldconfig(kernel_root, kernel_arch, kernel_config, file_name, action_name)
	if res_cmd == 0:
		if action_name == 'savedefconfig':
			diff_number = 0
		else:
	...
	for file_name in defconfig_list:
		...
		if res == 0:
			if option_file == 'KCONFIG':
				res = exec_oldconfig(kernel_root, kernel_arch, file_name, file_name, action_name)
