...
__copyright__ = "Copyright 2020, MediaTek Inc."
__version__   = "1.0.1"
...
class SignFlow(object):
    ...
    def process_squashfs(self, args, img):
        """
        copy the read-only part of a squashfs image to a new file
        with _ro.squashfs extension
        """
        ...
        if 'big' == sys.byteorder:
            byteorder = '>L'
        with open(in_img_path, 'rb') as fileobj:
            for chunk in iter(lambda: fileobj.read(4), ''):
                longval = struct.unpack(byteorder, chunk)[0]
                if SQUASHFS_MAGIC == longval:
                    print "Squashfs Magic Found"
                    magic_fnd = True
                    magic_idx = chunk_idx
                if magic_fnd:
                    if (magic_idx + offset) == chunk_idx:
                        squashfs_len = longval
                    if (magic_idx + offset + 1) == chunk_idx:
                        squashfs_len = (longval << 32 | squashfs_len)
                        print "Squashfs Length: " + hex(squashfs_len)
                        if 0 == squashfs_len:
                            print "Warning: Squash FS Read-Only Has Length Zero!"
                        break
                chunk_idx = chunk_idx + 1
        # create output file
        squashfs_len_bytes = bytearray()
        with open(in_img_path, 'rb') as fileobj:
            squashfs_len_bytes = fileobj.read()[magic_idx*4:magic_idx*4+squashfs_len]
        img_name = img.split('.')[0] + "_ro." + img.split('.')[1]
        out_img_path = os.path.join(self.in_path, img_name)
        with open(out_img_path, 'wb') as fileobj:
            fileobj.write(squashfs_len_bytes)
        return
    ...
    def sign_img_hash_list(self, args, img):
        ...
        if not os.path.exists(in_img_path):
            print "no " + str(img) + " image"
            return
        if '.' in img and "squashfs" == img.split('.')[1]:
            self.process_squashfs(args, img)
            img_name = img.split('.')[0].strip() + "_ro"
            in_img_path = os.path.join(self.in_path, img_name + ".squashfs")
        else:
            img_name = img.split('.')[0].strip()
        self.gen_cert2(args, in_img_path, img_name, 1)
        return

    def create_all_in_one_sig(self, args):
        """
        create allinone.sig. Make sure this function is called only after
        all images are signed and associated .sig files are generated.
        """
        allinone_sig_path = os.path.join(self.out_path, 'allinone.sig')
        if os.path.isfile(allinone_sig_path):
            os.remove(allinone_sig_path)
        allinone_sig_file = open(allinone_sig_path, 'wb')

        image_sig_dir_path = os.path.join(self.out_path, 'sig')

        for dir_name in os.listdir(image_sig_dir_path):
            if os.path.isdir(os.path.join(image_sig_dir_path, dir_name)):
                for file_name in os.listdir(os.path.join(image_sig_dir_path, dir_name)):
                    if file_name.endswith('.sig') is False:
                        continue
                    subimg_name = file_name.split('.')[0]
                    img_sig_path = os.path.join(image_sig_dir_path, dir_name, file_name)
                    img_hdr_path = os.path.join(image_sig_dir_path, dir_name, subimg_name + '_hdr.bin')
                    if os.path.isfile(img_hdr_path):
                        os.remove(img_hdr_path)
                    # create mkimg header configuration files
                    mkimg_cfg_name = subimg_name + '.cfg'
                    mkimg_cfg_path = os.path.join(image_sig_dir_path, dir_name, mkimg_cfg_name)
                    mkimg_cfg_file = open(mkimg_cfg_path, 'w')
                    mkimg_cfg_file.write('NAME = ' + subimg_name)
                    mkimg_cfg_file.close()
                    # create mkimg header
                    img_hdr = lib.mkimghdr.mkimage_hdr()
                    img_hdr.update_mkimage_hdr(None, mkimg_cfg_path)
                    img_hdr.pack()
                    img_hdr.output(None, img_hdr_path)
                    # concatenate mkimg header + sig file into allinone.sig
                    img_hdr_file = open(img_hdr_path, 'rb')
                    allinone_sig_file.write(img_hdr_file.read())
                    img_hdr_file.close()
                    img_sig_file = open(img_sig_path, 'rb')
                    allinone_sig_file.write(img_sig_file.read())
                    img_sig_file.close()

        allinone_sig_file.close()
    ...
    def sign_all_img(self, args, single_bin_dict, multi_bin_dict, img_hash_list):
        ...
        self.create_all_in_one_sig(args)
